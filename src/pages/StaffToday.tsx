...
export default function StaffToday() {
  ...
  const dayLabel = useMemo(() => formatDayRow(today), [today]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const products = (prodData ?? []) as Array<{ id: string; name: string; default_price_cents: number | null }>;
        ...
        const deliveryId = (delivs?.[0]?.id as string | undefined) ?? undefined;
        const savedNote = ((delivs?.[0]?.note as string | null) ?? "") as string;

        // items salvati (received_qty)
        ...
        // se esistono righe salvate, mostrale; altrimenti (per ora) 0: verrà riempito dal weekly_expected
        setReceived(() => {
          const next: Record<string, number> = {};
        ...
        // marca se è già compilato oggi
        if (deliveryId) {
          lastExpectedKeyRef.current = "LOCKED_BY_SAVED_DELIVERY";
        } else {
          lastExpectedKeyRef.current = "";
        }
      } catch (e) {
  ...
  // 2) carica weekly_expected per il giorno della settimana
  // se oggi NON è compilato, imposta received = expected così non resti a 0
  useEffect(() => {
    if (loading) return;
    ...
    const tick = async () => {
      try {
        ...
        const rows = (data ?? []) as Array<{ product_id: string; expected_qty: number | null }>;
        ...
        // se oggi NON era compilato (nessun salvataggio precedente), riempi received = expected
        const isTodayAlreadySaved = lastExpectedKeyRef.current === "LOCKED_BY_SAVED_DELIVERY";
  ...
  const save = async () => {
    try {
      ...
      // dopo un save, “blocchiamo” l’autofill da weekly_expected
      lastExpectedKeyRef.current = "LOCKED_BY_SAVED_DELIVERY";
    } catch (e) {
      // eslint-disable-next-line no-console
      console.error(e);
